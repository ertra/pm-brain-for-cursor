...
- [`create-product-brief`](../create-product-brief/SKILL.md) — if interviews surfaced a problem worth shipping, turn the findings into a brief. Reference your interview synthesis in the brief's Section B ("evidence") and Section C ("why now").
- [`competitive-landscape`](../competitive-landscape/SKILL.md) — validate whitespace or status-quo claims with users after the landscape analysis.
- [`critique-agent`](../critique-agent/SKILL.md) / [`critique-prd`](../critique-prd/SKILL.md) — pressure-test a brief or PRD that leans on interview claims; both look for `cited` vs `thin` evidence.
